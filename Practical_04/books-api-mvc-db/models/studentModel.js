...
async function getAllStudents() {
  ...
  try {
    ...
    const query = "SELECT id, name, age, email FROM Students";
...
async function getStudentById(id) {
  ...
  try {
    connection = await sql.connect(dbConfig);
    const query = "SELECT id, name, age, email FROM Students WHERE id = @id";
...
async function createStudent(studentData) {
  ...
  try {
    connection = await sql.connect(dbConfig);
    const query =
      "INSERT INTO Students (name, age, email) VALUES (@name, @age, @email); SELECT SCOPE_IDENTITY() AS id;";
    const request = connection.request();
